...
**タイムゾーン設定**: このアプリケーションは**日本標準時間（JST / Asia/Tokyo）**を使用します。`appsscript.json`のタイムゾーン設定が正しいことを確認してください。設定が異なると、日付計算が正しく動作しません。
...
#### appsscript.jsonの設定
1. 左側のファイルリストで⚙️（プロジェクトの設定）アイコンをクリック
2. 「appsscript.json」マニフェスト ファイルをエディタで表示する」にチェックを入れる
3. 左側のファイルリストに`appsscript.json`が表示されるのでクリック
4. このリポジトリの`appsscript.json`の内容をすべてコピー＆ペースト
5. **重要**: `"timeZone": "Asia/Tokyo"`が設定されていることを確認
6. `Ctrl+S`（または`Cmd+S`）で保存

#### Code.gsの配置
1. 左側のファイルリストで`Code.gs`をクリック
2. 既存のコードを削除
3. このリポジトリの`Code.gs`の内容をすべてコピー＆ペースト
